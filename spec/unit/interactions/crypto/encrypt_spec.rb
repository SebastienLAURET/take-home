...
RSpec.describe Crypto::Encrypt, type: :interaction do
  describe "#execute" do
    ...
    context "when using base64 algorithm" do
      ...
      it "calls Crypto::Base64::Encrypt interaction" do
        expect(Crypto::Base64::Encrypt).to receive(:run).with(hash_including(payload: payload.stringify_keys)).and_call_original
    ...
    context "when algorithm is not provided" do
      ...
      it "uses base64 as default algorithm" do
        expect(Crypto::Base64::Encrypt).to(
          receive(:run)
      ...
      it "returns encrypted payload with default algorithm" do
        result = outcome.result
